...
//go:generate go run -x github.com/cilium/ebpf/cmd/bpf2go -cc $BPF_CLANG -cflags $BPF_CFLAGS bpf uretprobe.c -- -I./headers
...
func main() {
	...
	for {
		...
		log.Printf("%s:%s return value: %s, comm:%s", binPath, symbol, unix.ByteSliceToString(event.Line[:]), unix.ByteSliceToString(event.Comm[:]))
	}
}
